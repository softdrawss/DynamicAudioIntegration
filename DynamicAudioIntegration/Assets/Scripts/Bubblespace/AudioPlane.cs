...
public class AudioPlane : MonoBehaviour
{
    ...
    protected void DrawRay(Vector3[] origins)
    {
        for (int i = 0; i < rays; i++)
        {
            ...
            //Vector3 direction = new Vector3(Mathf.Cos(_currentAngle), 0f, Mathf.Sin(_currentAngle));
            //Gizmos.DrawRay(transform.position, CalculatePoint(_currentAngle));
            //Debug.DrawRay(transform.position, CalculateDirection(_baseAngle) * _stepDistances[i], rayColor, 1.0f);
        }
