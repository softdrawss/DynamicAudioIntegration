...
public class AzimuthPlane : AudioPlane
{
    ...
    {
        _firstPoint = transform.position + GetDirection(0f) * _radius;
        _prevPoint = _firstPoint;

        for (int i = 1; i <= _segments; i++)
        {
            currentAngle = _incrementAngle * i * Mathf.Deg2Rad;
            _newPoint = transform.position + GetDirection(currentAngle) * _radius;
    ...
    public float GetAzimuth() { return _radius;  }
}
